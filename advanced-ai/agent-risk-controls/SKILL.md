...
description: Safety controls and guardrails for autonomous trading agents. Use when deploying agents that can place real orders, to enforce position limits, drawdown limits, and fail-safe defaults.
...
risk_config = LiveRiskConfig(
    max_position_pct=0.05, max_drawdown_pct=0.10,
    max_daily_trades=100, max_order_value=100_000,
)
...
- [ ] Fail-safe default is reduce exposure, never increase
- [ ] Tested with adversarial inputs (extreme sizes, NaN, negative prices)
